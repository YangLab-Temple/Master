				...
				for i in range(iterations):
					...
					if p != []:
						...
						if m + (1.0*PREC) >= rt + corr and m - (1.0*PREC) <= rt + corr:
							ten_hold += 1
	##printing out average 3d and sd of each bin
	templist = []
	bimodallist_temp = [np.array(i)/np.max(i) for i in bimodallist]
	for i in range(len(bimodallist_temp[0])):
		templist.append([])
		for j in bimodallist_temp:
			templist[i].append(j[i])
	print('average 3d')
	for i in templist:
		print(np.average(i))
	print('sd 3d')
